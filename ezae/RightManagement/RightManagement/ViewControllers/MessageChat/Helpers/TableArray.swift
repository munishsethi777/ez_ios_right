...
class TableArray {
    ...
    var mapTitleToMessages:[String:[MessageDetail]] = [String:[MessageDetail]]()
    ...
    func addObjectsFromArray(messages:[MessageDetail]) {
        for message in messages {
            self.addMessage(message: message)
        }
    }
    func removeObject(message:MessageDetail) {
        self.removeMessage(message: message)
    }
    func removeObjectsInArray(messages:[MessageDetail]) {
        for message in messages {
            self.removeMessage(message: message)
        }
    }
    ...
    func indexPathForMessage(message:MessageDetail) -> NSIndexPath {
    ...
    func addMessage(message:MessageDetail) {
        let key = self.keyForMessage(message: message)
    ...
    func removeMessage(message:MessageDetail) {
        let key = self.keyForMessage(message: message)
        var array = self.mapTitleToMessages[key]
        if array != nil {
